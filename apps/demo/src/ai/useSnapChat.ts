...
import { SNAP_SYSTEM_PROMPT } from "./systemPrompt";
import { createExecuteJsTool } from "./snapTool";
import type { SnapAccessors } from "./snapToolHelpers";

export type ChatStatus = "idle" | "streaming" | "error";

export type SnapChatOptions = {
...
export function useSnapChat(options: SnapChatOptions): SnapChatHandle {
    ...
    async (text: string) => {
      ...
      try {
        ...
        const result = streamText({
          model: languageModel,
          system: SNAP_SYSTEM_PROMPT,
          messages: toModelMessages(baseMessages),
          tools: { execute_js: createExecuteJsTool(accessorsRef.current) },
          stopWhen: stepCountIs(MAX_TOOL_STEPS),
          abortSignal: controller.signal,
          // Disable automatic retries — for BYOK in the browser, retrying on
          // 429/5xx just burns the user's quota and delays the error.
          maxRetries: 0,
        });
